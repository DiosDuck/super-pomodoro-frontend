...
export class SettingsService {
    ...
    private loadUserSettings(): Observable<Settings> {
        return this.http.get<SettingsHttp>('/api/pomodoro/settings').pipe(
            switchMap((settingsHttp) => {
            ...
            catchError(() => {
                ...
                return this.http
                    .put('/api/pomodoro/settings', settingsHttp)
                    .pipe(
                        switchMap(() => of(settings)),
                        catchError(() => {
                            this.toastService.addToast(
                                'Error creating settings, please refresh the page',
                                'error',
                                10,
                            );
                            return of(settings);
    ...
    private getLocalStorageSettings(): Settings {
        let data = this.localStorageService.getJsonParsed(
            POMODORO_SETTINGS_KEY,
        );
        if (
            data !== null &&
            'type' in data &&
            data.type === POMODORO_SETTINGS_KEY
        ) {
            return data;
        }
    ...
    private setLocalStorageSettings(settings: Settings): void {
        this.localStorageService.parseAndSet(POMODORO_SETTINGS_KEY, settings);
    }
